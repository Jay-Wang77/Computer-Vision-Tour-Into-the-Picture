function [surfaces,test] = output(estimatedVertex,recVertex,img)
%obtain  5 surfaces

surfaces={}; % store 5 surfaces
[m,n,p]=size(img);
test=max([m,n]/500);
...
% calculate Homographiematrix H of different surfaces using fitgeotrans
% create new coordidate of different surfaces and new outputimages
...
surfaces{1}=outputImage(min(coord(:,2)):max(coord(:,2)),min(coord(:,1)):max(coord(:,1)),:);
...
surfaces{2}=outputImage(min(coord(:,2)):max(coord(:,2)),min(coord(:,1)):max(coord(:,1)),:);
...
surfaces{3}=outputImage(min(coord(:,2)):max(coord(:,2)),min(coord(:,1)):max(coord(:,1)),:);
...
surfaces{4}=outputImage(min(coord(:,2)):max(coord(:,2)),min(coord(:,1)):max(coord(:,1)),:);
...
surfaces{5}=outputImage(min(coord(:,2)):max(coord(:,2)),min(coord(:,1)):max(coord(:,1)),:);
